...
if(isset($_POST['sifre_degistir_btn'])){
    header('Location: adminSifreDegistir.php');
}*/
?>
...
if(isset($_POST['duzenle_btn'])) {
   //session_start();
   $id = $_POST['duzenle_btn'];
   //echo $_POST['duzenle_btn'];
   $_SESSION['id'] = $_POST['duzenle_btn'];
   echo "<script>
  function open_me(h_rf)
  {
      window.open(h_rf,'_blank','width=750,height=550,toolbar=0,menubar=0,location=0,status=0,scrollbars=1,resizable=0,left=0,top=0');
    
      return false;
  }

  open_me('http://localhost:8080/Mas Proje/revizeSayfasi.php?value=$id');
  </script>";
...
if (isset($_POST['ekle_submit'])) {
 ...
 if ($stmt = $conn->prepare("INSERT INTO urunsistemtablo (Kullanici_ID, Sistem, Birim, KartNo, SeriNo, ArizaTipi, UrunAdi, Sehir, GirisTarihi, ProjeAdi, ResimYol, ResimAD, EkBilgi) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")) {
    ...
    // Bind the variables to the parameter as strings. 
    $stmt->bind_param("sssssssssssss", $kullanici_id, $sistem, $birim, $kart_no, $seri_no, $ariza_tipi, $urun_adi, $sehir, $giris_tarihi, $proje_adi, $upload_image, $folder, $ek_bilgi);
...
if (isset($_POST['ara_btn']))
{
 ...
 if ($_POST['ara_txtbox'] != ''){
    ...
    if ($stmt = $conn->prepare("SELECT Sistem, Birim, KartNo, SeriNo, UrunAdi, ArizaTipi, Sehir, GirisTarihi, ProjeAdi, EkBilgi, ResimYol FROM urunsistemtablo WHERE KartNo=?")) {
 
       // Bind a variable to the parameter as a string. 
       $stmt->bind_param("s", $ara_txtbox);
  ...
  <div class="container-fluid">
    <div class="navbar-header">
      ...
      <a class="navbar-brand">
        <img src="/Mas Proje/images/admin.jpg">
      </a>
      
<a id="navbar-links" style="font-size: 24px">Hoşgeldiniz Admin</a>
    ...
    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
     <form method="post" action="">
      <ul class="nav navbar-nav">
        ...
        <li><a id="navbar-links" href="adminAyarlar.php">Ayarlar </a>

        </li>
        <li><a id="navbar-links" href="adminLogout.php">Çıkış </a>
...
<li>
    <input type="text" name="kullanici_id" class="field-style field-split align-left" placeholder="Kullanıcı ID" required/> 
</li>
...
<li>
	    <input type="date" name="tarih" class="field-style field-split align-none" required/>
</li>

<li>
     <input type="text" name="proje_adi" class="field-style field-split align-right" placeholder="Proje Adı" required/>
     <input type="file" name="resim" class="field-style field-split align-none" accept=".png, .jpg, .jpeg" required>
...
$fetch = mysql_query("SELECT * FROM urunsistemtablo LIMIT $startrow, 10")or
die(mysql_error());
